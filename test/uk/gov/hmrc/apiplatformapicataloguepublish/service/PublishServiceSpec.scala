...
//    }
    ...
    def commonStubs (isYaml: Boolean) ={
    ...
    def primePublishFail(isYaml: Boolean,  failedResult : ApiCatalogueFailedResult) ={
     commonStubs(isYaml)

      when(mockCatalogueConnector.publishApi(any[String])).thenReturn(Future.successful(Left(failedResult)))
